...
const { MenuItem, Order } = require("./models.js");
...
async function postMenuItem(newMenuItem) {
  ...
  console.log(`menuItem inserted with the _id: ${result.insertedId}`);
...
async function postOrder(newOrder) {
  ...
  console.log(`order inserted with the _id: ${result.insertedId}`);
}
